  ...
  return (
    ...
    >
      ...
      {/* Brief loading state while the on-books snapshot for the date loads — the manual entry
          modal below only mounts once that fetch resolves (see the effect above), so there's a
          short gap between choosing "Unesi ručno" and the form appearing. */}
      ...
      {mode === "import" && openDay !== null && openDate && (
        <ImportReportModal
          ...
          onFileSelected={(file, allMonths, parseError) => {
            ...
            // same file independently (a separate pair of sheets — it can succeed even when the
            // aggregate parse fails, or vice versa), and never blocks the aggregate import either.
            // Both misses are surfaced the same way, never silent.
